...
tab << journaliste.sample(5).to_s

tab[0] = tab[0].tr('"','').tr('[]','').split(',')
i = 0

while i < tab[0].size

	client.update("Salut #{tab[0][i]} !")
	i = i + 1
end

end

def like(client) #permet de liker les tweet qui ont le #bonjour_monde

  client.favorite(client.search("#bonjour_monde", result_type: "recent").take(20))

end

def follow(client) #permet de follow les tweet qui ont le tweet avec le #bonjour_monde
